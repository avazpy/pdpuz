...
async def phone_number_handler(msg: types.Message):
    ...
    if not await User.objects.filter(phone_number=phone).aexists() and msg.from_user.username and not await User.objects.filter(
        ...
        await msg.answer(f"link in site http://localhost:8005/")
